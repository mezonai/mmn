...
func NewVotor(validatorID string, privKey ed25519.PrivateKey, eventReceiver <-chan votor.VotorEvent, eventSender chan<- votor.VotorEvent, ln *p2p.Libp2pNetwork) *Votor {
	return &Votor{
...
func (v *Votor) tryNotar(blockInfo votor.BlockInfo) bool {
	...
	vote := &consensus.Vote{
		Slot:      slot,
		VoteType:  consensus.NOTAR,
		BlockHash: hash,
		VoterID:   v.validatorID,
	}
	vote.Sign(v.privKey)
...
func (v *Votor) tryFinal(slot uint64, hash [32]byte) {
	_, isNotarized := v.blockNotarized[slot]
	_, isVotedNotar := v.votedNotar[slot]
	_, isBadWindow := v.badWindow[slot]

	if isNotarized && isVotedNotar && !isBadWindow {
		vote := &consensus.Vote{
			Slot:      slot,
			VoteType:  consensus.FINAL,
			BlockHash: hash,
			VoterID:   v.validatorID,
		}
		vote.Sign(v.privKey)
