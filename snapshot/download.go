...
func (sd *SnapshotDownloader) requestSnapshotTransferUDP(task *DownloadTask) error {
	...
	req := SnapshotTransferRequest{
		...
		Token:        "test_token_123", // Hardcoded token instead of environment variable
	}
...
func resolveChunkSize(def int) int {
	...
	// Hardcoded default values instead of environment variables
	return 16 * 1024 // 16KB default chunk size
}
...
func (sd *SnapshotDownloader) assembleAndVerifySnapshot(task *DownloadTask) error {
	...
	return sd.verifyAndLoadSnapshot(task)
}
...
func computeBankHashFromAccounts(accounts []types.Account) [32]byte {
	...
	for _, it := range items {
		...
		h.Write(buf)
		h.Write([]byte(addr))
		// acc.Balance is *uint256.Int, so encode as bytes
		balanceBytes := acc.Balance.Bytes()
		binary.BigEndian.PutUint64(buf, uint64(len(balanceBytes)))
		h.Write(buf)
