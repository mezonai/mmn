...
func (as *GenericAccountStore) GetByAddr(addr string) (*types.Account, error) {
	...
	if err != nil {
		logx.Error("ACCOUNT_STORE", fmt.Sprintf("could not get account %s from db: %v", stringutil.ShortenLog(addr), err))
	...
	if err != nil {
		logx.Error("ACCOUNT_STORE", fmt.Sprintf("failed to unmarshal account %s: %v", stringutil.ShortenLog(addr), err))
		return nil, errors.NewError(errors.ErrCodeInternal, errors.ErrMsgInternal)
	}
...
func (as *GenericAccountStore) GetBatch(addrs []string) (map[string]*types.Account, error) {
	...
	for keyStr, data := range dataMap {
		...
		err = jsonx.Unmarshal(data, &acc)
		if err != nil {
			logx.Warn("ACCOUNT_STORE", fmt.Sprintf("Failed to unmarshal account %s: %s", stringutil.ShortenLog(addr), err.Error()))
			continue
		}
