...
async function verifyBalanceAcrossNodes(nodeClients: NodeClient[], address: string, expectedBalance: number): Promise<boolean> {
...
describe('Parallel Three Nodes Token Transfer Tests', () => {
  ...
  beforeAll(async () => {
    ...
    console.log('Connected to nodes:', nodeClients.map(nc => nc.name));
  });
  ...
  describe('Multi-Node Parallel Transfer Tests', () => {
    test('Parallel Transfers Across 3 Nodes with Consensus Verification', async () => {
      ...
      const totalBalance = (finalSenderBalance || 0) + (finalRecipient1Balance || 0) + 
                          (finalRecipient2Balance || 0) + (finalRecipient3Balance || 0);
    ...
    test('Concurrent Same-Sender Transactions to Different Nodes', async () => {
      ...
      const transactions = recipients.map((recipient, index) => {
        const tx = buildTx(sender.publicKeyHex, recipient.publicKeyHex, 300, `Concurrent tx ${index + 1}`, index + 1, TxTypeTransfer);
      ...
      console.log('Concurrent transaction responses:', responses.map((r, i) => ({
    ...
    test('Node Failure Resilience', async () => {
      ...
      for (let i = 0; i < numTransactions; i++) {
        const tx = buildTx(sender.publicKeyHex, recipient.publicKeyHex, 50, `Resilience test ${i}`, i + 1, TxTypeTransfer);
        tx.signature = signTx(tx, sender.privateKey);
      ...
      console.log(`✅ Node failure resilience test passed (${successfulTxs}/${numTransactions} transactions succeeded)`);
     }, 90000);
    });
  });