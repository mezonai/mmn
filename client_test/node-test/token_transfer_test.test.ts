  ...
  try {
    ...
    if (expectedTransactions && expectedTransactions.length > 0) {
      ...
      for (let i = 0; i < Math.min(expectedTransactions.length, history.txs.length); i++) {
        ...
        console.log(
          `  Expected: ${expected.sender.substring(0, 8)}... -> ${expected.recipient.substring(0, 8)}... (${expected.amount
          })`
        );
        console.log(
...
describe('Token Transfer Tests', () => {
  ...
  describe('Success Cases', () => {
    test('Valid Transfer Transaction', async () => {
      ...
      console.log('Sender:', sender.publicKeyHex, sender.privateKey.export({ format: 'der', type: 'pkcs8' }).toString('hex'));
      console.log('Recipient:', recipient.publicKeyHex);
