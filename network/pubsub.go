...
func (ln *Libp2pNetwork) SetupCallbacks(ld *ledger.Ledger, privKey ed25519.PrivateKey, self config.NodeConfig, bs *blockstore.BlockStore, collector *consensus.Collector, mp *mempool.Mempool) {
	ln.SetCallbacks(
		...
		func(txData []byte) error {
			logx.Info("TRANSACTION", "Received transaction from network: ", txData)
			mp.AddTx(txData, false) // false = don't broadcast again
...
func (ln *Libp2pNetwork) TxBroadcast(ctx context.Context, txData []byte) error {
	...
	if ln.topicTxs != nil {
		ln.topicTxs.Publish(ctx, data)
	}

	ln.BroadcastToStreams(data, ln.txStreams)
